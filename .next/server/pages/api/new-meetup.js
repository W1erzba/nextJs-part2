...
async function hanlder(req, res) {
    ...
    if (req.method === 'POST') {
        try {
            const data = req.body;
            const client = await external_mongodb_namespaceObject.MongoClient.connect('mongodb+srv://Wierzba:wiadrokrowa1@cluster0.gfze6w8.mongodb.net/meetups?retryWrites=true&w=majority');
            const db = client.db();
